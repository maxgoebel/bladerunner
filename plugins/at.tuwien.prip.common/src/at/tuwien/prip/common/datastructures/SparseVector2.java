 ...
 */
public class SparseVector2 implements Serializable, Cloneable {
  /**
	 * 
	 */
...
/** Contains a double value associated to this Sparse Vector */
  ...
  /** Duplicates a vector */
  public SparseVector2(SparseVector2 v) {
    label=v.label;
    dim=v.dim;
    size=v.size;
    if(v.val==BINARY) {
  ...
  public SparseVector2(double l, List<Integer> dimensions) {
    label=l;
  ...
  public SparseVector2(double l, int[] d) {
    Arrays.sort(d);
    // Count zeroes
    int zeroes=0;
  ...
  public SparseVector2(double[] v,int[] d) {
    this(0.0,d,v);
  }

  /** Creates a Sparse Vector from (sorted) dimensions, (modifiable) values and a label */
  public SparseVector2(double l,int[] d, double[] v) {
    val=v;
    dim=d;
    label=l;
    size=dim.length;
  ...
  public SparseVector2(double label, double... values) {
    this.label=label;
    val=values;
  ...
  public double sprod(SparseVector2 v) {
    int i=-1;
    int j=-1;
    double r=0.0;
    while(true) {
  ...
  public double cosine(SparseVector2 v) {
  ...
  public static String visualize(SparseVector2... vectors) {
    return(visualize(Arrays.asList(vectors)));
  }

  /** Visualizes two-dimensional vectors */
  public static String visualize(List<SparseVector2> vectors1, List<SparseVector2> vectors2) {
    List<SparseVector2> l=new ArrayList<SparseVector2>();
    l.addAll(vectors1);
    l.addAll(vectors2);
    return(visualize(l));
  }
  ...
  /** Visualizes two-dimensional vectors */
  public static String visualize(SparseVector2[] vectors1, SparseVector2[] vectors2) {
    List<SparseVector2> l=new ArrayList<SparseVector2>();
  ...
  /** Visualizes two-dimensional vectors */
  public static String visualize(List<SparseVector2> vectors) {
    double xmax=0;
    double xmin=0;
    double ymax=0;
    double ymin=0;
    for(SparseVector2 v : vectors) {
  ...
  public interface Distance {
    public double distance(SparseVector2 v1, SparseVector2 v2);
  }
  ...
  public static void kMeans(SparseVector2[] dots, SparseVector2[] centers) {
    kMeans(dots, centers,eucledianDistance,0.1,10);
  }

  /** Does a simple K-means until the number of iterations is exceeded*/
  public static void kMeans(SparseVector2[] dots, SparseVector2[] centers, Distance distanceFunction, double epsilon, int iterations) {
    while(iterations-->0) {
      ...
      SparseVector2[] oldCenters=new SparseVector2[centers.length];
      for(int center=0;center<centers.length;center++) {
        oldCenters[center]=centers[center];
        centers[center]=new SparseVector2(centers[center].label,0);
      }
  ...
  public double eucledianDistance(SparseVector2 v) {
    int myIndex=0;
    int vIndex=0;
    double result=0;
  ...
  public SparseVector2 add(SparseVector2 v) {
  ...
  public SparseVector2 multiply(double r) {
    if(isBinary()) {
      val=new double[size()];
