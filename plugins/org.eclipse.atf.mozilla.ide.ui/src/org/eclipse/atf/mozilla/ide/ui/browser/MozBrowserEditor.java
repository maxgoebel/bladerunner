...
public class MozBrowserEditor extends EditorPart implements IWebBrowser, ISelectionProvider, ISelectionListener {
	...
	* and Windows, that is the 'Ctrl' key.  For Mac OS X, it is 'Cmd' (Apple) key. 
	...
	protected SelectionBox selectionBox = null; //a new instance created per Document loaded
	...
	protected class MozillaBrowserListener implements ProgressListener, LocationListener, StatusTextListener, nsIDOMEventListener {
		...
		public void init() {
			...
			getMozillaBrowser().addDisposeListener(new DisposeListener() {
				...
				public void widgetDisposed(DisposeEvent e) {
					...
					getMozillaBrowser().removeProgressListener(MozillaBrowserListener.this);
		...
		public void changed(LocationEvent event) {
			...
			if (event.top) {
				...
				setTitleToolTip(event.location);

				// show the current URL in the navBar
				navBar.setLocationURL(event.location);
				...
				domEventsPublisher.locationChanged(event.location);
			}
		}
		...
		public void completed(ProgressEvent event) {
			...
			// URL was changed, but change haven't completed: URL was incorrect or couldn't connect
			if (!changed) {
				// on Windows, browser is not refreshed when fil
				browser.redraw();
			...
			if (event.widget instanceof Browser) {
				...
				windowEventTarget.addEventListener(DOM_CONTENT_LOADED, this, true);
				...
				boolean isInitiated = page.getReference(MozBrowserEditor.this).getPart(false) != null;
				if (isInitiated && !page.isPartVisible(MozBrowserEditor.this)) {
					page.bringToTop(MozBrowserEditor.this);
				}
		...
		public void handleEvent(nsIDOMEvent event) {
			...
			if (PAGESHOW.equals(eventType) || DOM_CONTENT_LOADED.equals(eventType)) {
				...
				if (isTop) {
					...
					try {
						event.getCurrentTarget().removeEventListener(DOM_CONTENT_LOADED, this, true);
					} catch (XPCOMException e) {
						// ignore
					}
				...
				DOMDocumentEvent e = new DOMDocumentEvent(MozBrowserEditor.this, documentEventTarget, isTop, DOMDocumentEvent.LOAD_EVENT);
				domEventsPublisher.documentLoaded(e);
				...
				final DOMDocumentEvent e = new DOMDocumentEvent(MozBrowserEditor.this, documentEventTarget, isTop, DOMDocumentEvent.UNLOAD_EVENT);
				domEventsPublisher.documentUnloaded(e);
	...
	public MozBrowserEditor() {
		MozIDEUIPlugin.getDefault().getApplicationEventAdmin().postEvent(new BrowserCreatedEvent(this));
	}
	...
	public void createPartControl(Composite parent) {
		...
		/*
		 * There are serveral ways to initiate the close of this editor so the
		...
		goToURL(((MozBrowserEditorInput) getEditorInput()).getURL());
	...
	protected IJavaScriptConsole createJavaScriptConsole() {
		...
		final nsIDOMEventListener atfConsoleLogListener = new nsIDOMEventListener(){
			...
			public void handleEvent(nsIDOMEvent event) {
				...
				try{
					//MozIDEUIPlugin.debug( "print firebug!" );	
					...
					//MozIDEUIPlugin.debug( node.getFirstChild().getNodeValue() );
	...
	public void dispose() {
		MozIDEUIPlugin.getDefault().getApplicationEventAdmin().postEvent(new BrowserDisposedEvent(this));
	...
	protected void revealSelection() {
		...
		if (selectedNode.getNodeType() == nsIDOMNode.ELEMENT_NODE) {
			...
			try {
				...
				nsIDOMNSHTMLElement nsElement = (nsIDOMNSHTMLElement) selectedNode.queryInterface(nsIDOMNSHTMLElement.NS_IDOMNSHTMLELEMENT_IID);
	...
	/*
	 * The following set of listeners are used to detect changes on the DOM stucture and
		...
		public void handleEvent(nsIDOMEvent event) {
			...
			try {
				...
				if (internalFilter.isATFInternal(mutEvent.getTarget())) {
					...
					domEventsPublisher.fireNodeInserted(mutEvent);
				}
			} catch (Exception e) {
				// do nothing if QI fails
				MozIDEUIPlugin.log(e);
		...
		public void handleEvent(nsIDOMEvent event) {
			...
			try {
				...
				if (internalFilter.isATFInternal(mutEvent.getTarget())) {
					...
					domEventsPublisher.fireNodeRemoved(mutEvent);
				}
			} catch (Exception e) {
				// do nothing if QI fails
				MozIDEUIPlugin.log(e);
			...
			try {
				...
				if (internalFilter.isATFInternal(mutEvent.getTarget())) {
					...
					if (mutEvent.getAttrChange() == nsIDOMMutationEvent.ADDITION) {
						domEventsPublisher.fireAttributeAdded(mutEvent);
					} else if (mutEvent.getAttrChange() == nsIDOMMutationEvent.REMOVAL) {
						domEventsPublisher.fireAttributeRemoved(mutEvent);
					} else if (mutEvent.getAttrChange() == nsIDOMMutationEvent.MODIFICATION) {
						domEventsPublisher.fireAttributeModified(mutEvent);
					}
				}
