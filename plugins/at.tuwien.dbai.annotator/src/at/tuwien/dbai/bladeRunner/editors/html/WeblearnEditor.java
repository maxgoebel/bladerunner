...
IModelChangedListener
{
	...
	protected LearningSystemHighlighter highlighter;
	...
	public WeblearnEditor(Composite parent, boolean standalone)
	{
		...
		this.highlighter = new LearningSystemHighlighter(new HighlightState());
...
//    	manager.add(clickSelectAction);
	...
	public void fireDocumentLoaded(ProgressEvent event)
	{
		Display.getCurrent().asyncExec(new Runnable() {
			...
			public void run()
			{
				HTMLBenchmarkDocument doc = (HTMLBenchmarkDocument) we.getActiveDocument();
...
//		navBar.addExtensionAction( clearCacheAction );
	...
	public void documentUpdated(DocumentUpdateEvent ev)
	{
		...
		String s2 = null;
		if (ev.getDocumentUpdate().getProvider()!=null) {
			ev.getDocumentUpdate().getProvider().getClass().getCanonicalName();
		}
...
//			IDocument document = DocumentController.docModel.getDocument();
	 ...
	 * @param color
	 */
	public void highlightNodes(RGB color, List<Element> nodes)
	{
		highlighter.addColor2Nodes(color, nodes);
	}
	 ...
	 * @param items
	 */
	public void highlightSelections(List<AbstractSelection> items)
	{
		HighlightState hs = new HighlightState();
		hs.setItems(items);
		highlighter.setHS(hs);
